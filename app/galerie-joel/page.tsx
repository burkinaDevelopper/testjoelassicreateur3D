...
export default function page() {
    ...
    const galeryJoels: Galery[] = useStoreGaley((s) => s.galeryJoels);
    ...
    const current = selectedIndex !== null ? galeryJoels[selectedIndex] : undefined;
    const hasPrev = selectedIndex !== null && selectedIndex > 0;
    const hasNext = selectedIndex !== null && selectedIndex < galeryJoels.length - 1;

    const close = () => setSelectedIndex(null);
    const showPrev = () => setSelectedIndex((i) => (i !== null && i > 0 ? i - 1 : i));
    const showNext = () => setSelectedIndex((i) => (i !== null && i < galeryJoels.length - 1 ? i + 1 : i));
    ...
    <div className="min-h-screen bg-[#0E0E0E]">
          ...
          <main className="max-w-screen-xl mx-auto px-4 py-10 lg:py-14">
            <h1 className="text-white text-2xl lg:text-3xl font-black mb-6">
              Galerie <span className="text-[#F0B90B]">Joel</span>
            </h1>

            {galeryJoels.length === 0 ? (
              <p className="text-zinc-400 text-center py-20">Aucune image pour le moment.</p>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {galeryJoels.map((image, index) => {
                  const width = 639;
                  const height = 638;
                  return (
                  <button
                    ...
                    <>
                      ...
                      <div className="mt-4 text-center">
                        ...
                        <p className="text-zinc-500 text-xs mt-1">
                          {selectedIndex !== null ? selectedIndex + 1 : 0} / {galeryJoels.length}
                        </p>
                      </div>
                    </>
                  )}
