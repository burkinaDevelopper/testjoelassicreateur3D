'use client'
import { Maximize2, Pause, Play, SkipBack, SkipForward, Volume2, Volume1, VolumeX, CheckCircle } from 'lucide-react'
...
export default function VideoPlayer() {
  ...
  const isCompleted = useMemo(() => {
    if (!userChapter?.chapter?.modules || !currentLessonId) return false
    for (const mod of userChapter.chapter.modules) {
      const les = mod.lessons?.find((l: any) => l.id === currentLessonId)
      if (les) return les.user_progress?.status === 'completed'
    }
    return false
  }, [userChapter, currentLessonId])

  const currentModData = useMemo(() => {
    if (!userChapter?.chapter?.modules || !currentLessonId) return null
    for (const mod of userChapter.chapter.modules) {
      const hasLesson = mod.lessons?.some((l: any) => l.id === currentLessonId)
      if (hasLesson) return { modId: mod.id, lessons: mod.lessons }
    }
    return null
  }, [userChapter, currentLessonId])

  const handleFinish = async () => {
    if (!currentModData || finishing) return
    setFinishing(true)
    try {
      const lessonRes = await axios.put(`/api/lessons/status/${currentLessonId}`, { status: 'completed' })
      if (lessonRes?.status === 201 && currentLessonId) {
        updateLessonStatus(currentLessonId, 'completed')
      }

      const allLessonsCompleted = currentModData.lessons.every((l: any) =>
        l.id === currentLessonId || l.user_progress?.status === 'completed'
      )

      if (allLessonsCompleted) {
        const modRes = await axios.put(`/api/modules/status/${currentModData.modId}`, { status: 'completed' })
        if (modRes?.status === 201) updateModuleStatus(currentModData.modId, 'completed')
      }
    } catch {
      if (currentLessonId) updateLessonStatus(currentLessonId, 'inactived')
    } finally {
      setFinishing(false)
    }
  }
  ...
  const finishButton = (
    <button
      ...
      disabled={isCompleted || finishing}
      className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold transition-all self-end ${
        isCompleted
          ? 'bg-green-100 text-green-600 dark:bg-green-900/30 dark:text-green-400 cursor-default'
          : finishing
          ? 'bg-gray-100 text-gray-400 dark:bg-dark-700 cursor-wait'
          : 'bg-green-500 hover:bg-green-600 text-white cursor-pointer'
      }`}
    >
      <CheckCircle className="w-4 h-4" />
      {isCompleted ? 'Terminé' : finishing ? 'En cours…' : 'Marquer comme terminé'}
    </button>
  )
